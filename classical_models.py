...
class NonLinear(nn.Module):
    ...
    def forward(self, x, W=None):
        ...
        if batched:
            ...
            x1 = torch.bmm(w1, x.transpose(1, 2)).transpose(1, 2).to(device) + b1.unsqueeze(1).to(device)
            x1 = torch.relu(x1)
            return torch.bmm(w2, x1.transpose(1, 2)).transpose(1, 2).to(device) + b2.unsqueeze(1).to(device)
