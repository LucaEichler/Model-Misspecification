...
class Dataset(nn.Module):
    ...
    def __init__(self, dim: int, min_len: int, max_len: int, min_param_len: int, max_param_len: int,
                 mode: str = 'fixed', device=torch.device('cpu')):
...
class Gaussian(Dataset):
    ...
    def __init__(self, dim: int, min_len: int, max_len: int, min_param_len: int, max_param_len: int,
                 variance: str = 'identity', mode: str = 'fixed', device=torch.device('cpu')):
...
class LinearRegression(Dataset):
    ...
    def __init__(self, dim: int, min_len: int, max_len: int, min_param_len: int, max_param_len: int,
                 x_sampling: str = 'uniform', mode: str = 'fixed', device=torch.device('cpu'), beta: float = 1.):
...
class LinearClassification(Dataset):
    ...
    def __init__(self, dim: int, min_len: int, max_len: int, min_param_len: int, max_param_len: int,
                 num_classes: int = 2, temperature: float = 1., x_sampling: str = 'uniform', mode: str = 'fixed',
...
class NonLinearRegression(Dataset):
    ...
    def __init__(self, dim: int, min_len: int, max_len: int, min_param_len: int, max_param_len: int, h_dim: int,
                 n_layers: int, act, x_sampling: str = 'uniform', mode: str = 'fixed', device=torch.device('cpu'),
                 beta: float = 0.0001):
    ...
    def conditional_fn(self, x: torch.Tensor, params: torch.Tensor, params_mask: torch.Tensor):
        ...
        for _ in range(self.n_layers - 1):
            h = self.act(torch.matmul(h,
                                      params[:, start: start + self.h_dim * self.h_dim].reshape(batch_size, self.h_dim,
                                                                                                self.h_dim)))
...
nlin = NonLinear(dx=1, dy=1, dh=10)
res = nlin(x_train.transpose(0,1), torch.cat((params, torch.zeros_like(params[:, :1])),dim=-1))
...
class NonLinearClassification(Dataset):
    ...
    def __init__(self, dim: int, min_len: int, max_len: int, min_param_len: int, max_param_len: int, h_dim: int,
                 n_layers: int, act, num_classes: int = 2, temperature: float = 1., x_sampling: str = 'uniform',
                 mode: str = 'fixed', device=torch.device('cpu')):
    ...
    def conditional_fn(self, x: torch.Tensor, params: torch.Tensor, params_mask: torch.Tensor):
        ...
        h = torch.matmul(h, params[:, start: start + self.h_dim * self.num_classes].reshape(batch_size, self.h_dim,
                                                                                            self.num_classes))
        h = h.transpose(1, 0).contiguous()
        return h
...
class GMM(Dataset):
    ...
    def __init__(self, dim: int, n_mixtures: int, min_len: int, max_len: int, min_param_len: int, max_param_len: int,
                 variance: str = 'identity', sample_mixture_weights: bool = False, mode: str = 'fixed',
                 device=torch.device('cpu')):
    ...
    def sample_variance(self, batch_size: int):
        ...
        '''
        if self.variance == 'identity':
            var = 0.1 * torch.eye(self.dim).unsqueeze(0).unsqueeze(0).repeat(batch_size, self.n_mixtures, 1, 1).to(
                self.device)
            ...
            var = self.var_prob.rsample((batch_size * self.n_mixtures,), max_try_correction=1000).to(self.device).view(
                batch_size, self.n_mixtures, self.dim, self.dim)
    ...
    def get_batch(self, batch_size: int, length: int = None):
        ...
        mean.transpose(1, 0).reshape(batch_size, -1), (param, pi)), mask.byte(), params_mask.byte()